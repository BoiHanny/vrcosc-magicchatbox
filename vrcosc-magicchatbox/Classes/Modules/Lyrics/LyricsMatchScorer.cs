...
public static class LyricsMatchScorer
{
    ...
    public static LyricsMatch PickBest(IReadOnlyList<LyricsCandidate> candidates, LyricsQuery query)
    {
        ...
        {
            double score = Score(candidates[i], query);
            if (score >= AcceptThreshold && score > best.Score)
    ...
    public static double Score(LyricsCandidate candidate, LyricsQuery query)
    {
        ...
        double title = Similarity(candidate.TrackName, query.Title);
        ...
        double duration = DurationScore(candidate.DurationSeconds, query.Duration);
        if (duration <= 0)
            return 0;
    ...
    public static double DurationScore(double candidateSeconds, TimeSpan queried)
    {
