    ...
    public static class OSCSender
    {
        ...
        public static async Task SendOSCMessage(bool FX, int delay = 0)
        {
            ...
            try
            {
                ...
                if(oscSender != null &&
                    (ViewModel.Instance.OSCIP != oscSender.Address || ViewModel.Instance.OSCPortOut != oscSender.Port))
                {
                    oscSender.Close();
                    oscSender = null;
                }
                ...
                if(SecOscSender != null &&
                    (ViewModel.Instance.OSCIP != SecOscSender.Address ||
                        ViewModel.Instance.SecOSCPort != SecOscSender.Port))
                {
                    oscSender.Close();
                    oscSender = null;
                }
                ...
                await Task.Run(
                    () =>
                    {
                        ...
                        } else
                        {
                            ...
                            {
                                SecOscSender.Send(
                                    new OscMessage("/chatbox/input", ViewModel.Instance.OSCtoSent, true, FX));
                            }
                        }
                    });
            } catch(Exception ex)
        ...
        public static async Task ToggleVoice(bool force = false)
        {
            ...
            if(ViewModel.Instance.MasterSwitch &&
                !ViewModel.Instance.AutoUnmuteTTS ||
                !force &&
                !ViewModel.Instance.MasterSwitch)
                return;
