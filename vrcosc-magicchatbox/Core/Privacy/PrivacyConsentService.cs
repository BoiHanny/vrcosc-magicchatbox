...
public sealed class PrivacyConsentService : IPrivacyConsentService
{
    ...
    public ConsentState GetState(PrivacyHook hook) => hook switch
    {
        ...
        PrivacyHook.SharedLayoutImport => Settings.SharedLayoutImportConsent,
    ...
    private void SetState(PrivacyHook hook, ConsentState newState)
    {
        lock (_stateLock)
        {
            switch (hook)
            {
                ...
                case PrivacyHook.SharedLayoutImport:
                    Settings.SharedLayoutImportConsent = newState;
                    Settings.SharedLayoutImportDecidedAt = DateTime.UtcNow;
                    break;
                default:
